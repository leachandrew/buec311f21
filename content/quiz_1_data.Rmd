...
```
The average over all three sections was `r round(mean(test_data$grade,na.rm=T),2)`/10.

The averages for the individual sections were `r round(mean(test_A1$grade,na.rm=T),2)`/10 for section A1, `r round(mean(test_A2$grade,na.rm=T),2)`/10 for section A2 and `r round(mean(test_A3$grade,na.rm=T),2)`/10 for section A3.
...
samples<-ggplot(test_data%>%filter(!is.na(grade)),aes(grade,group=samples))+
  ...
  labs(x="Range of Scores",y="Probability Density",
       title="Distribution of quiz scores by number of samples completed",
       caption="Kernel-smoothed so you won't necessarily find your actual grade",
       fill=guide_legend(title="Number of/nSamples/nCompleted"),color=guide_legend(title="Number of/nSamples/nCompleted"))
